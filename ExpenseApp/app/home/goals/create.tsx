...
const GoalCreate = () => {
  ...
  const [selectedIcon, setSelectedIcon] = React.useState<string>(ICONS[0]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const raw = (data as any)?.goal ?? (data as any)?.data ?? (data as any);
      const goalId = String(raw?.id ?? raw?._id ?? "");
      if (goalId) {
        try {
          ...
          console.warn("Could not schedule goal notifications", err);
        }
      }
      if (vibrationEnabled)
